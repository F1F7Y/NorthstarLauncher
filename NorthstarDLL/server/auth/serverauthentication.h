...
class ServerAuthenticationManager
{
  public:
	...
	std::unordered_map<R2::CClient*, PlayerAuthenticationData> m_PlayerAuthenticationData;
  ...
  public:
	...
	void AddPlayer(R2::CClient* pPlayer, const char* pAuthToken);
	void RemovePlayer(R2::CClient* pPlayer);

	bool VerifyPlayerName(const char* pAuthToken, const char* pName, char pOutVerifiedName[64]);
	bool IsDuplicateAccount(R2::CClient* pPlayer, const char* pUid);
	bool CheckAuthentication(R2::CClient* pPlayer, uint64_t iUid, char* pAuthToken);

	void AuthenticatePlayer(R2::CClient* pPlayer, uint64_t iUid, char* pAuthToken);
	bool RemovePlayerAuthData(R2::CClient* pPlayer);
	void WritePersistentData(R2::CClient* pPlayer);
};
